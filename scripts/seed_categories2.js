...
async function seedCategories() {
    ...
    // Let's get movies that are NOT the absolute newest.
    // Or just fetch a lot and take the LAST ones.
    const moviesSnap = await db.collection('movies').limit(1000).get();
    
    const docs = moviesSnap.docs.reverse(); // Reverse so we take from the back
